...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        try {
                ...
                case 'remove':
                    ...
                    collector.on('collect', m => {
                        ...
                        if (index < 0 || index >= data.fav.length) {
                            return message.channel.send('Invalid number. Please enter a number from the list above.');
                        }
                    ...
                    collector.on('end', collected => {
                        if (collected.size === 0) {
                            message.channel.send('Timed out. No song removed.');
                        }
                    });
                    break;
            ...
            message.channel.send('```Error processing favorites command```');
        }
    }
};
