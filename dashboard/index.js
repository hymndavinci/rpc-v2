...
module.exports = (clients) => {
    const client = clients[0]; // alias for single-client routes
    ...
    setInterval(() => {
        ...
        for (const [token, session] of activeSessions) {
            if (now - session.createdAt > 24 * 60 * 60 * 1000) activeSessions.delete(token);
        }
    }, 3600000);

    // Login Page
    app.get('/login', (req, res) => {
        if (req.cookies.auth_token && activeSessions.has(req.cookies.auth_token)) {
            return res.redirect('/');
        }
        res.render('login');
    });
    ...
    app.post('/api/login', async (req, res) => {
        ...
        res.cookie('auth_token', sessionToken, { httpOnly: true, sameSite: 'strict', maxAge: 24 * 60 * 60 * 1000 });
        res.json({ success: true });
    });

    // Auth Middleware (Protects everything below)
    app.use((req, res, next) => { req.client = clients[parseInt(req.cookies.active_account) || 0] || clients[0]; res.locals.accountCount = clients.length; if (!req.client) return res.send('Waiting for bots to load...');
    ...
    app.get('/logout', (req, res) => {
        if (req.cookies.auth_token) activeSessions.delete(req.cookies.auth_token);
        res.clearCookie('auth_token');
        res.redirect('/login');
    });

    app.post('/switch-account', (req, res) => {
        const idx = parseInt(req.body.index) || 0;
        res.cookie('active_account', idx, { maxAge: 24 * 60 * 60 * 1000 });
    ...
    app.get('/', (req, res) => {
        ...
        res.render('index', {
            ...
            commandCount: req.client.commands ? req.client.commands.size : 0,
            latency: req.client.ws.ping
        });
    });

    app.get('/api/latency', (req, res) => {
        res.json({ latency: req.client.ws.ping });
    });
    ...
    app.post('/update-status', async (req, res) => {
        try {
            ...
            await rpcManager.setPresence(req.client, rpcManager.loadData());
    ...
    // --- QUEST ROUTES ---
    // Store multiple QuestManagers based on the client token
    const questManagers = new Map();
    function getQuestManager(token) {
        if (!questManagers.has(token)) {
            questManagers.set(token, new QuestManager(token));
        }
    ...
    app.get('/quest', (req, res) => {
        ...
        res.render('quest', { user: req.client.user, page: 'quest' });
    });

    // --- Quest API: routes match frontend calls in quest.ejs ---
    app.post('/api/quest/start', (req, res) => {
        getQuestManager(req.client.token).startAll(); // Async background
        res.json({ success: true, message: 'Bot started doing quests in the background.' });
    });

    // Frontend calls /quest/start-all → alias to /api/quest/start logic
    app.post('/quest/start-all', (req, res) => {
        getQuestManager(req.client.token).startAll();
        res.json({ success: true, message: 'Quest started.' });
    });

    app.post('/api/quest/stop', (req, res) => {
        getQuestManager(req.client.token).stopAll();
        res.json({ success: true, message: 'Quest process stopped.' });
    });

    // Frontend calls /quest/stop-all → alias to stop logic
    app.post('/quest/stop-all', (req, res) => {
        getQuestManager(req.client.token).stopAll();
        res.json({ success: true, message: 'Quest stopped.' });
    });

    app.post('/quest/clear-logs', (req, res) => {
        const questManager = getQuestManager(req.client.token);
        if (questManager.clearLogs) questManager.clearLogs();
        res.json({ success: true });
    });

    app.get('/quest/status', (req, res) => {
        const questManager = getQuestManager(req.client.token);
        res.json({
            logs: questManager.globalLogs || [],
            isRunning: questManager.isRunning || false
        });
    });

    // Frontend polls /api/quests for logs + isRunning
    ...
    app.get('/music', (req, res) => {
        if (!req.client.user) return res.send('Bot loading...');
        res.render('music', { user: req.client.user, page: 'music' });
    });

    // Guard: semua /api/music/* butuh Lavalink aktif
    app.use('/api/music', (req, res, next) => {
        if (!req.client.lavalink) {
            return res.json({ success: false, message: 'Lavalink tidak dikonfigurasi. Cek LAVALINK_* di .env' });
        }
        next();
    });
    ...
    app.post('/api/music/join', async (req, res) => {
        ...
        try {
            ...
            if (!gid || !cid) {
                return res.status(400).json({ success: false, message: 'Server dan channel wajib dipilih' });
            }
            const guild = req.client.guilds.cache.get(gid);
            if (!guild) {
                return res.status(404).json({ success: false, message: 'Bot tidak ada di server itu' });
            }
            ...
            if (req.client.ws && req.client.ws.shards && req.client.ws.shards.size > 0) {
                const sid = guild.shardId != null ? guild.shardId : 0;
                let shard = req.client.ws.shards.get(sid);
                if (!shard && req.client.ws.shards.size > 0) shard = req.client.ws.shards.first();
                if (!shard) {
                    return res.status(500).json({ success: false, message: 'WebSocket shard tidak tersedia' });
                }
                shard.send(payload);
            } else if (typeof req.client.ws.broadcast === 'function') {
                req.client.ws.broadcast(payload);
            } else {
                req.client.ws.send(payload);
            }

            res.json({ success: true });
        } catch (e) {
            console.error(e);
            res.status(500).json({ success: false, message: e.message || 'Gagal join voice' });
        }
    });
    ...
    app.post('/api/music/leave', async (req, res) => {
        ...
        try {
            const gid = guildId != null ? String(guildId) : '';
            if (gid) {
                req.client.queueManager.delete(gid);
                if (req.client.lavalink) await req.client.lavalink.destroyPlayer(gid);
            }

            const payload = {
                op: 4,
                d: { guild_id: gid, channel_id: null, self_mute: false, self_deaf: false },
            };

            if (gid) {
                const guild = req.client.guilds.cache.get(gid);
                if (req.client.ws && req.client.ws.shards && req.client.ws.shards.size > 0) {
                    const sid = guild && guild.shardId != null ? guild.shardId : 0;
                    let shard = req.client.ws.shards.get(sid);
                    if (!shard && req.client.ws.shards.size > 0) shard = req.client.ws.shards.first();
            ...
            console.error(e);
            res.status(500).json({ success: false, message: e.message || 'Gagal leave' });
        }
    });
    ...
    app.post('/api/music/play', async (req, res) => {
        ...
        try {
            ...
            const result = await playLogic(req.client, guildId, query); // Use req.client (per-session account)
            res.json(result);
