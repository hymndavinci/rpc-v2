            ...
            if (res.loadType === 'search' || res.loadType === 'track') {
                ...
                if (tracks && tracks.length > 0) {
                    ...
                    const existingTitles = new Set([
                        ...queue.history.map(t => t.info.title.toLowerCase()),
                        ...queue.songs.map(t => t.info.title.toLowerCase()),
                        queue.nowPlaying ? queue.nowPlaying.info.title.toLowerCase() : null
                    ]);

                    const newTracks = tracks.filter(t =>
                        !existingUris.has(t.info.uri) &&
                        !existingTitles.has(t.info.title.toLowerCase())
                    );
