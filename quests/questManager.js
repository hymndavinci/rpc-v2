...
class QuestManager {
    ...
    sleep(ms) {
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                this.abortControllers.delete(cancel);
                resolve();
            }, ms);
            const cancel = () => {
                clearTimeout(timer);
    ...
    async doingQuest(quest) {
        ...
        if (!this.stopped) {
            this.log(quest.id, quest.isCompleted() ? '✅ Quest Completed!' : '⏹ Quest task ended.');
        }
    }
    ...
    async runVideo(quest, taskConfig, taskName) {
        ...
        while (current < target && !this.stopped) {
            ...
            try {
                const jitter = Math.floor(Math.random() * 3); // 0-2s natural jitter
                const res = await this.videoProgress(quest.id, Math.floor(current) + jitter);
                if (res.completed_at) {
                    this.log(quest.id, 'COMPLETED!');
                    quest.updateUserStatus(res.user_status || res);
                    break;
                }
